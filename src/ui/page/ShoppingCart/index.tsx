...
export default function ShoppingCart() {
  const [shoppingCartDataList, setShoppingCartDataList] = useState<
    ShoppingCartData[] | undefined | null
  >(undefined);
  ...
  const onApiUpdateCartItemQuantity = (
    isSuccess: boolean,
    product_id?: number,
    cartQuantity?: number
  ) => {
  ...
  const calTotalPrice = (): number => {
    ...
    if (shoppingCartDataList) {
      for (const shoppingCartItemData of shoppingCartDataList) {
        totalPrice +=
          shoppingCartItemData.price * shoppingCartItemData.cart_quantity;
      }
    }
    return totalPrice;
  };
  ...
  return (
    <div>
      ...
      <Card.Header as="h5" className={"shopping-cart-header"}>
        Shopping Cart Item
      </Card.Header>
      ...
      <div className={"total-part"}>
        ...
        <div className={"checkout-part"}>
          <button
            ...
            aria-label="Left Align"
            onClick={(event) => {
              handleCheckoutOnClick(event);
            }}
          >
            <span>
              <FontAwesomeIcon
                icon={solid("money-bill-1")}
                style={{ color: "white" }}
              />{" "}
              Confirm
            </span>
